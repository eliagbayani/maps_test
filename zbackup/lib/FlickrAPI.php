...
$GLOBALS['expire_seconds'] = false; //0 -> expires now, false -> doesn't expire, 172800 -> expires in 2 days
...
class FlickrAPI
{
    ...
    public static function get_taxa_for_photo($photo_id, $secret, $last_update, $auth_token = "", $user_id = NULL)
    {
        ...
        foreach($photo->tags->tag as $tag)
        {
            ...
            {
                $vernacular = self::stripNonValidXMLCharacters(trim($arr[1]));
                if(Functions::is_utf8($vernacular) && $vernacular) $parameters["commonNames"][] = new \SchemaCommonName(array("name" => $vernacular));
            }
    ...
    private function format_utf8($string)
    {
        ...
        if(!Functions::is_utf8($string)) return utf8_encode($string);
        return $string;
    }
    
    private function fix_latin1_mangled_with_utf8_maybe_hopefully_most_of_the_time($str)
    {
        // XML 1.0
        ...
        $str = preg_replace($xml10pattern, "", $str);
        $str = preg_replace($xml11pattern, "", $str);
        return $str;
    ...
    private function utf8_encode_callback($m)
    {
    ...
    private Function stripNonValidXMLCharacters($in) 
    {
        $out = (string) '';
    ...
    public static function get_photostream_photos($auth_token = "", $resource_file = null, $user_id = NULL, $start_year = NULL, $months_to_be_broken_down = NULL, $max_photos_per_taxon = NULL)
    {
        ...
        $sciname = "Butirinus argenteus Jerdon, 1849";
        $sciname = "Gadus morhua Linnaeus";
        $sciname = "Squatarola squatarola";
        // $sciname = "Panthera tigris";
        if(self::is_sciname_synonym($sciname)) echo "\n[$sciname is synonym]\n";
        else echo "\n[$sciname is accepted name]\n";
        exit("\nstop here\n");
        */
