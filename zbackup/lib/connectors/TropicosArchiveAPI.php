...
class TropicosArchiveAPI
{
    function __construct($folder)
    {
        ...
        $this->download_options = array('resource_id' => 218, 'cache_path' => '/Volumes/Eli black/eol_cache_tropicos/', 'expire_seconds' => false, 
    ...
    private function process_fields($records, $class)
    {
        ...
        */
        ...
        foreach($records as $rec)
        {
            ...
            {
                if(!isset($unique_ids[$c->field_name_of_unique_id]))
                {
                    $unique_ids[$c->field_name_of_unique_id] = '';
                    $this->archive_builder->write_object_to_file($c);
                }
            }
            else $this->archive_builder->write_object_to_file($c); //for measurementorfact
    ...
    {
        $temp_archive_batch_count = 10000; //debug orig is 10k
        $k = 0;
        $i = 0;
        $j = 0;
