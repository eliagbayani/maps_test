...
// these two variables are used to differentiate trusted and unreviewed content
$GLOBALS['ingest_unreviewed_contentYN'] = true;
$GLOBALS['agents_with_trusted_images'] = array(); // this will be populated by a spreadsheet


class FlickrAPI
{
    public static function get_all_eol_photos($auth_token = "", $resource_file = null, $options = array())
    {
        $user_id    = @$options['user_id'];
        $start_date = @$options['start_date'];
        $end_date   = @$options['end_date'];
        if(isset($options['ingest_unreviewed_contentYN'])) $GLOBALS['ingest_unreviewed_contentYN'] = $options['ingest_unreviewed_contentYN'];
        if($GLOBALS['ingest_unreviewed_contentYN'] == false) $GLOBALS['agents_with_trusted_images'] = self::get_agents_list_with_trusted_images();
    ...
    private function get_agents_list_with_trusted_images()
    {
        $agents = array();
        if($sheet = Functions::get_google_spreadsheet(array("spreadsheet_title" => "BOLD image mappings")))
        {
            foreach($sheet[2] as $rec) $agents[] = trim($rec);
            $agents[] = "Dick Culbert";
        }
        print_r($agents); exit;
        return $agents;
    ...
    public static function get_data_objects($photo, $user_id)
    {
        ...
        if($GLOBALS['ingest_unreviewed_contentYN'] == false) // means we're only ingesting trusted images
        {
            if(!in_array($photo->owner->username, $GLOBALS['agents_with_trusted_images']))
            {
                print "\n untrusted agent: " . $photo->owner->username;
                return array();
            }
        }
        else // means we're ingesting unreviewed images
        {
            if(in_array($photo->owner->username, $GLOBALS['agents_with_trusted_images']))
            {
                print "\n trusted agent: " . $photo->owner->username;
                return array();
            }
        }
    ...
    public static function get_photostream_photos($auth_token = "", $resource_file = null, $user_id = NULL, $start_year = NULL, $months_to_be_broken_down = NULL, $max_photos_per_taxon = NULL)
    {
        ...
        foreach($date_range as $range)
        {
            ...
            $taxa = self::get_all_eol_photos($auth_token, $resource_file, array("user_id" => $user_id, "start_date" => $range["start_timestamp"], "end_date" => $range["end_timestamp"]));
